...
def evaluate_next_steps_and_fork_path(grid, ongoing_paths, already_visited_position, start_position, end_position):
    ...
    for current_path in ongoing_paths:
        ...
        if current_path_row_index - 1 >= 0:
            if abs(grid[current_path_row_index][current_path_col_index] - grid[current_path_row_index - 1][
                current_path_col_index]) <= 1:
        ...
        if current_path_col_index + 1 < len(grid[0]):
            if abs(grid[current_path_row_index][current_path_col_index] - grid[current_path_row_index][
                current_path_col_index + 1]) <= 1:
...
def feed_possible_start_point(grid, ongoing_paths):
    row_index = 0
    for row in grid:
        col_index = 0
        for col in row:
            if grid[row_index][col_index] == 1:
                start_position_name = str(row_index) + '/' + str(col_index)
                ongoing_paths.append({'step': 0, 'position': start_position_name, 'trace': start_position_name})
            col_index += 1
        row_index += 1
    return ongoing_paths.copy()
...
def get_fast_route_to_ending(grid, start_position, end_position):
    ...
    ongoing_paths = feed_possible_start_point(grid, ongoing_paths)
    arrived_at_destination = False
    shortest_path = 0
