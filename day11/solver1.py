    ...
    for round in range(0, round_count):
        for monkey_id in range(0, len(monkey_items)):
            for item in monkey_items[monkey_id]['items']:
                ...
                item_after_operation = (lambda old: eval(monkey_items[monkey_id]['operation']))(item)
                item_after_getting_bored = item_after_operation // 3
                if item_after_getting_bored % monkey_items[monkey_id]['test_divide'] == 0:
                    monkey_items[monkey_items[monkey_id]['target_monkey_if_true']]['items'].append(item_after_getting_bored)
                else:
                    monkey_items[monkey_items[monkey_id]['target_monkey_if_false']]['items'].append(item_after_getting_bored)
...
def solve():
    with open('input/input1', 'r') as file:
        ...
        result1, result2 = get_two_most_active_monkey_item_count(20, monkey_items)
