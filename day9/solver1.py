def process_tail_step(visited_positions, current_head_position, current_tail_position):
    ...
    position_id = str(current_tail_position[0]) + "/" + str(current_tail_position[1])
    #print(position_id)
    if position_id not in visited_positions:
        visited_positions.add(position_id)
    return visited_positions, current_tail_position
...
def process_head_move(visited_positions, current_head_position, current_tail_position, line):
    ...
    for step in range(0, int(line_split[1])):
        ...
        visited_positions, current_tail_position = process_tail_step(visited_positions, current_head_position, current_tail_position)
...
def solve():
    with open('input/input1', 'r') as file:
        ...
        for line in file:
            visited_positions, current_head_position, current_tail_position = process_head_move(visited_positions, current_head_position, current_tail_position, line)
